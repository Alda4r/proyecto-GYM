...
public class ProgresoController {
    ...
    public String verProgreso(@RequestParam(value = "buscarEmail", required = false) String buscarEmail, 
        ...
        if (isAdmin) {
            List<Usuario> listaAlumnos = usuarioRepository.findAll().stream()
            ...
            if (buscarEmail != null && !buscarEmail.isEmpty()) {
                Usuario alumnoSeleccionado = usuarioRepository.findById(buscarEmail).orElse(null);
    ...
    private void cargarMetricasUsuario(Usuario usuario, Model model) {
        ...
        double estaturaPromedio = 1.70; 
        double imc = usuario.getPesoActual() / (estaturaPromedio * estaturaPromedio);
        ...
        List<HistorialEntrenamiento> historial = historialRepository.findByUsuarioEmail(usuario.getEmail());
        ...
        List<Comida> comidasHoy = comidaRepository.findByUsuarioEmailAndFecha(usuario.getEmail(), LocalDate.now());
